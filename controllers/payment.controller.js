...
export const paymentSuccess = async (req, res) => {
  try {
    ...
    // ✅ Fixed: Redirect to frontend routes with URL parameters
    console.log("🔄 Redirecting user to frontend success page")
    return res.redirect(`${process.env.FRONTEND_URL}/payment-success/${tran_id}`)
  } catch (error) {
    console.error("❌ Payment success handler error:", error)
    return res.redirect(`${process.env.FRONTEND_URL}/payment-fail/error`)
  }
}
...
export const paymentCancel = async (req, res) => {
  try {
    ...
    if (!tran_id) {
      console.log("❌ Transaction ID missing in cancel handler")
      return res.redirect(`${process.env.FRONTEND_URL}/payment-cancel/unknown`)
    }
    ...
    // ✅ Fixed: Redirect to frontend routes with URL parameters
    return res.redirect(`${process.env.FRONTEND_URL}/payment-cancel/${tran_id}`)
  } catch (error) {
    console.error("❌ Payment cancel handler error:", error)
    return res.redirect(`${process.env.FRONTEND_URL}/payment-cancel/error`)
  }
}
