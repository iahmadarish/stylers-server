...
// // Admin routes
// router.use(protect)
// router.get("/admin-list", getAllProductsForAdmin)
...
// --- CORS OPTIONS Handling (Unchanged) ---
router.options("/", (req, res) => {
  res.setHeader("Access-Control-Allow-Origin", req.headers.origin || "*");
  res.setHeader("Access-Control-Allow-Methods", "GET, POST, PUT, DELETE, PATCH, OPTIONS");
  res.setHeader("Access-Control-Allow-Headers", "Content-Type, Authorization, X-Requested-With, Accept, Origin");
...
// ----------------------------------------------------
// PUBLIC ROUTES (All GET routes defined first)
// ----------------------------------------------------
router.get("/search", searchProducts)
router.get("/filter", filterProducts)
router.get("/featured", getFeaturedProducts)
...
router.route("/:id")
  .patch(protect, restrictTo("admin", "executive"), productUpload.array("images", 10), updateProduct)
  .delete(protect, restrictTo("admin", "executive"), deleteProduct)
